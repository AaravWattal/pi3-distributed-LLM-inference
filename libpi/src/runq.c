// Adapted from: https://github.com/karpathy/llama2.c/blob/master/runq.c
...
void malloc_run_state(RunState* s, Config* p) {
    ...
    //printk("dim: %d, n_kv_heads: %d, n_heads: %d, seq_len: %d\n", p->dim, p->n_kv_heads, p->n_heads, p->seq_len);
...
// ----------------------------------------------------------------------------
// Basic implementations of standard library functions

// Simple implementation of absolute value for floats
float fabs(float x) {
    return x < 0.0f ? -x : x;
}

// Simple implementation of round function for floats
// Rounds to the nearest integer value, with halfway cases rounded away from zero
float round(float x) {
    // Handle positive and negative cases separately to ensure correct rounding direction
    if (x >= 0.0f) {
        // For positive numbers, adding 0.5 and truncating gives the rounded value
...
void read_checkpoint(char* checkpoint, Config* config, TransformerWeights* weights,
    ...
    if (file->n_data < sizeof(Config) + 2*sizeof(int) + sizeof(uint32_t) + sizeof(uint8_t)) {
        panic("Checkpoint file too small");
    }
    
    // Parse the file data manually
    void *ptr = file->data;
    
    // Read magic number (uint32)
    uint32_t magic_number = *((uint32_t*)ptr);
    if (magic_number != 0x616b3432) { // "ak42" in ASCII
        panic("Bad magic number");
    }
    ptr += sizeof(uint32_t);
    
    // Read version number (int)
    int version = *((int*)ptr);
    ptr += sizeof(int);
    if (version != 2) {
        panic("Bad version %d, need version 2", version);
    }
    
    // Skip to header size
    int header_size = 256; // Fixed for version 2
    ...
    // Read shared_classifier and group_size
    ...
    // Set file size and data pointers
    *file_size = file->n_data;
    *data = (float*)file->data;
    
    // Skip header bytes to get to weights
    float *weights_ptr = (float*)(((char*)file->data) + header_size);
...
void notmain(void) {
    ...
    #if CACHING
    setup_vm();
    //printk("setup vm done, about to init pi sd\n");
    #endif
    pi_sd_init();
    //printk("pi sd init done\n");
    

    #if VFP
    //printk("about to enable vfp\n");
    enable_vfp();
    //printk("enabled vfp\n");
    #endif
