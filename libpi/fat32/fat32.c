...
fat32_fs_t fat32_mk(mbr_partition_ent_t *partition) {
  ...
  // from the partition
  unsigned lba_start = partition->lba_start;
  // the start LBA + the number of reserved sectors
  unsigned fat_begin_lba = lba_start + boot_sector.reserved_area_nsec;
  // the beginning of the FAT, plus the combined length of all the FATs
  unsigned cluster_begin_lba = fat_begin_lba + (boot_sector.nfats * boot_sector.nsec_per_fat);
  // from the boot sector
  unsigned sec_per_cluster = boot_sector.sec_per_cluster;
  // from the boot sector
  unsigned root_first_cluster = boot_sector.first_cluster;
  // from the boot sector
  unsigned n_entries = (boot_sector.nsec_per_fat * boot_sector.bytes_per_sec) / 4;
...
static uint32_t get_cluster_chain_length(fat32_fs_t *fs, uint32_t start_cluster) {
  ...
  while (1) {
    ...
    if (fat32_fat_entry_type(cur_cluster) == LAST_CLUSTER) {
      break;
    }
...
static void read_cluster_chain(fat32_fs_t *fs, uint32_t start_cluster, uint8_t *data) {
  ...
  while (1) {
    uint32_t lba = fs->cluster_begin_lba + (cur_cluster - 2) * fs->sectors_per_cluster;
    pi_sd_read(data + (i * bytes_per_cluster), lba, fs->sectors_per_cluster);
...
static fat32_dirent_t *get_dirents(fat32_fs_t *fs, uint32_t cluster_start, uint32_t *dir_n) {
  ...
  uint32_t n_bytes = chain_len * fs->sectors_per_cluster * boot_sector.bytes_per_sec;
  *dir_n = n_bytes / sizeof(fat32_dirent_t);
  uint8_t* buf = kmalloc(n_bytes);
...
pi_directory_t fat32_readdir(fat32_fs_t *fs, pi_dirent_t *dirent) {
  ...
  pi_dirent_t* return_dirent = kmalloc(n_dirents * sizeof(pi_dirent_t));
  ...
  for (uint32_t i = 0; i < n_dirents; i++) {
    ...
    return_dirent[n_valid_dirents] = pi_dirent;
    n_valid_dirents++;
  }
...
static int find_dirent_with_name(fat32_dirent_t *dirents, int n, char *filename) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    fat32_dirent_name(cur_dirent, cur_filename);
...
pi_dirent_t *fat32_stat(fat32_fs_t *fs, pi_dirent_t *directory, char *filename) {
  ...
  int found_idx = find_dirent_with_name(dirents, dir_n, filename);
  ...
  *dirent = dirent_convert(dirents + found_idx);
  return dirent;
}
...
pi_file_t *fat32_read(fat32_fs_t *fs, pi_dirent_t *directory, char *filename) {
  ...
  uint32_t file_start_cluster = (dirents[found_idx].hi_start << 16) | dirents[found_idx].lo_start;
  uint32_t cluster_len = get_cluster_chain_length(fs, file_start_cluster);

  // TODO: allocate a buffer large enough to hold the whole file
  uint32_t file_buf_bytes = cluster_len * fs->sectors_per_cluster * boot_sector.bytes_per_sec;
  uint8_t* file_buf = kmalloc(file_buf_bytes);

  // TODO: read in the whole file (if it's not empty)
  read_cluster_chain(fs, file_start_cluster, file_buf);

  // TODO: fill the pi_file_t
  pi_file_t *file = kmalloc(sizeof(pi_file_t));
  *file = (pi_file_t) {
    .data = file_buf,
    .n_data = dirents[found_idx].file_nbytes,
    .n_alloc = file_buf_bytes,
  };
  return file;
}
