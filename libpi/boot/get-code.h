...
uint32_t get_code(void) {
    ...
    if (boot_get32() != PUT_PROG_INFO) {
        panic("Did not get PUT_PROG_INFO\n");
    }
    ...
    //       - libpi/memmap 
    //    for definitions.
    uint32_t addr_top = addr + nbytes;
    uint32_t put32_addr = (uint32_t)&PUT32;
    uint32_t prog_end = (uint32_t)__prog_end__;

    if (!(addr_top < put32_addr || prog_end < addr)) {
        boot_err(BOOT_ERROR, "collision");
    }
    ...
    if (crc32((void *)addr, nbytes) != cksum) {
        boot_err(BOOT_ERROR, "incorrect cksum");
    }

    // 7. send back a BOOT_SUCCESS!
    boot_putk("Willy Zhu: success: Received the program with UART!");
